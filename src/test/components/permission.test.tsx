...
describe('权限组件', () => {
  it('Can：有权限渲染 children，没权限渲染 fallback', () => {
    render(
      <PermissionProvider permissions={['customer:view']}>
        <Can permission="customer:view">
          <span>新建</span>
        </Can>
        <Can permission="customer:export" fallback={<span>导出（无权限）</span>}>
          <span>导出</span>
        </Can>
      </PermissionProvider>,
    )
  ...
  it('Can：通配权限同样生效', () => {
    render(
      <PermissionProvider permissions={['customer:*']}>
        <Can permission="customer:export">
          <span>导出</span>
        </Can>
      </PermissionProvider>,
    )
  ...
  it('RequirePermission：没有权限时渲染 403，而不是页面内容', () => {
    render(
      <PermissionProvider permissions={['report:view']}>
